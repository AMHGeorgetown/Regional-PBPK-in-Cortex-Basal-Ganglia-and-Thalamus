...
Fucsf = 0.995   //; fraction unbound in CSF
fup   =  0.1   //; fraction unbound in plasma
BP    = 0.61   //; blood to plasma ratio
fumic = 1   //; fraction unbound in microsomes
mw    = 252.3   //; Molecular weight
logp  = 2.47  //; n-octanol lipophilicity

acid   = 1 //; Drug is monoprotic acide (1= yes, 0= no)  
...
CLrenal   =  0 //; CLint renal (mL/hr)
CLhep= 0  //(HLM_CLint/fumic)*MPPGL*Vli*(60)*1000; // CLint scaled (mL/hr)
CLbrain = 0  //2 % of CLhep 

// {Absorption}
oral = 0  // 1 for yes, 0 for no
IV   = 0 //; 1 for yes , 0 for no
IRa  = 0//; infusion rate Dose/Time (hr-1) always check units 1 g dose over 3 minutes; Dose input in R is 1e6/0.05 hour 
Abs = 0//; observed rate of oral absorption
Kpf=0
...
double Kpad = 2.078*Kpf;  //; adipose

double Kpgu = 1.624*Kpf;   //; gut
double Kpki = 1.286*Kpf;   //; kidney
double Kpli = 1.268*Kpf;   //; liver
double Kpmu = 0.541 *Kpf;  //; muscle
double Kpsp = 0.584*Kpf;   //; spleen
double Kplp = 1*Kpf; //; low perfusion

double Kpbrec1 = 0.627;  //; Cortex cb to ecf 
double Kpbric1 = 0.6392;  //; Ecf to ICF


double Kpbrec3 =  0.792; //; BG cb to ecf 
double Kpbric3 =  0.8062;  //; Ecf to ICF



double Kpbrec4 = 0.734; //; TH cb to ecf 
double Kpbric4 = 0.7477;   //; Ecf to ICF

double Kpbrec5 =  0.802; //; ROB cb to ecf 
double Kpbric5 =  0.8166;   //; Ecf to ICF
...
double SAbcm4 = Fsabcm4 * Ticf4 * 10000 ; //SA brain cell membrane -> converted from micrometer to mL
double SAlyso4 = Fsalyso4 * Ticf4 * 10000; //SA lysosomal membrane -> converted from micrometer to mL
...
double cl3 = (QTbcsfb + QPbcsfb) * 60 * IF2 ;  //; from choroid plexus blood to CSF
...
double Venous = 
  ...
  Qbr4*(Ccb4) +   Qbr5*(Ccb5) + (IV * Absorption) + Qcsf*Csas

   ;

double Absorption = oral *(Abs*D) + IV * (D*IRa);
...
capture Csa = (Asas/Vsas); // SAS in mcg/mL 
...
capture CortTis = (Acb1+Aecf1+Aicf1+Alyso1)/ (Vcb1+Vecf1+Vicf1+Vlyso1);


capture CortTis2 = (Acb1/Vcb1) +(Aecf1/Vecf1) + (Aicf1/Vicf1)+(Alyso1/Vlyso1);
...
[ CAPTURE ] Cvenous Cp Cpm Mball  amt Csa Csam Ceca Cec1 Cec2 Cec3 Cec4 Icf1 Icf2 Icf2 
			...
			Ccb1 Ccb2 Ccb3 Ccb4 Ccb5 Bra Cmva CortTis CortTis2
