...
//{CSF}
FVcb2 =   0.0003 //; cerebral bood - CSF
...
WIroi1 = 0.00005  //; Width Paracellular RO1 1
WIroi2 = 0.00005  //; Width Paracellular RO1 2
WIroi3 = 0.00005  //; Width Paracellular RO1 3
WIroi4 = 0.00005  //; Width Paracellular RO1 4




WIbcsfb = 0.00005//; Width BCSFB/BBB -> distance to travel, not gaps b/w cells
...
//  IF is set to 1 and EF is adjusted
...
Fucsf = 0.997   //; fraction unbound in CSF
fup   = 0.65  //; fraction unbound in plasma
BP    = 1.02  //; blood to plasma ratio
fumic = 1   //; fraction unbound in microsomes
mw    = 285.4   //; Molecular weight
logp  = 0.87   //;  lipophilicity

acid   = 0 //; Drug is monoprotic acide (1= yes, 0= no)  
base   = 1 //; Drug is monoprotic base  (1=yes, 0 = no)
...
Ka = 0  //; Ka (hr-1)
F  =0//; fraction absorbed
 

[ MAIN  ]
//{KP Factors}
double Kpad = 2.026*Kpf;  //; adipose

double Kpgu = 1.957*Kpf;   //; gut
double Kpki = 2.069*Kpf;   //; kidney
double Kpli = 1.775*Kpf;   //; liver
double Kpmu = 1.646*Kpf;  //; muscle
double Kpsp = 1.624*Kpf;   //; spleen
double Kplp = 3*Kpf;   //; low perfusion

double Kpbrec1 = 1.089*Kpf; //; Cortex cb to ecf 
double Kpbric1 = 1.073*Kpf; //; Ecf to ICF


double Kpbrec3 = 1.254*Kpf; //; BG cb to ecf 
double Kpbric3 = 1.211*Kpf ; //; Ecf to ICF

double Kpbrec4 = 1.248*Kpf;  //; TH cb to ecf
double Kpbric4 = 1.117*Kpf ;  //; Ecf to ICF

double Kpbrec5 = 1.269*Kpf; //; ROB cb to ecf 
double Kpbric5 = 1.214*Kpf;  //; Ecf to ICF


// {Total tissue volumes - mL}

double CLtotal= CLrange *(BW/1000) *60; 
double CLhep= 0.89 * CLtotal ;  //Clearance is calculated from value ml/min*kg/ units are mL/hr
double CLrenal   =  0.11 * CLtotal    ;//; Renal clearance is 11% of total/hepatic
...
double PHF11 = acid * ((1+(pow(10,(pHp - pka1)))) / (1+(pow(10, (pHecf-pka1))))) + 
             ...
             dpbase * ((1 +(pow(10, pka2 - pHp) + pow(10,(pka1 + pka2 - 2 * pHp))))/
                        1 +(pow(10, pka2 - pHecf) + pow(10,(pka1 + pka2 - 2 * pHecf)))); //; ionization factor from ECF to CB
...
double PHF2 = acid * ((1+(pow(10,(pHp - pka1)))) / (1+(pow(10, (pHcsf-pka1))))) + 
             ...
             dpbase * ((1 +(pow(10, pka2 - pHp) + pow(10,(pka1 + pka2 - 2 * pHp))))/
                        1 +(pow(10, pka2 - pHcsf) + pow(10,(pka1 + pka2 - 2 * pHcsf)))) ; //; ionization factor from CSF to CB
...
double PHF41 = acid * ((1+(pow(10,(pHicf - pka1)))) / (1+(pow(10, (pHlyso-pka1))))) + 
             ...
             dpbase * ((1 +(pow(10, pka2 - pHicf) + pow(10,(pka1 + pka2 - 2 * pHicf))))/
                        1 +(pow(10, pka2 - pHlyso) + pow(10,(pka1 + pka2 - 2 * pHlyso)))) ; //; Ionization factor from Lyso to ICF
double PHF42 = PHF41 ; 
double PHF43 = PHF41 ; 
double PHF44 = PHF41 ; 
...
double Venous = 
  ...
  Qlp*(Clp/Kplp*BP) +
  Qbr1*(Ccb1) + Qbr2 * (Ccb2) + Qbr3 * (Ccb3) +
  Qbr4*(Ccb4) +   Qbr5*(Ccb5) + Qcsf*Csas+ (IV * Absorption)

   ;

double Absorption = oral *(Ka*D*F) + IV * (D*IRa);
...
//{Brain}
dxdt_Acb1 = Qbr1 * (Carterial - Ccb1) - (cl11 *Ccb1*BP) + (cl21 *(Cecf1/Kpbrec1)) ; // Cerebral blood
...
dxdt_Acb3 = Qbr3 * (Carterial - (Ccb3)) - (cl12 * Ccb3*BP) + (cl22 *(Cecf2/Kpbrec3)) ; // Cerebral blood
...
dxdt_Acb4 = Qbr4 * (Carterial - (Ccb4)) - (cl13 * Ccb4*BP) + (cl23 * (Cecf3/Kpbrec4)) ; // Cerebral blood
...
dxdt_Acb5 = Qbr5 * (Carterial - (Ccb5)) - (cl14 * Ccb5*BP ) + (cl24 * Cecf4/Kpbrec5) ; // Cerebral blood
...
//{CSF}
dxdt_Acb2 = Qbr2 * (Carterial - (Ccb2)) - (cl3 * Ccb2*BP) + (cl4 * Cvent)  ; // CB to ventricles
...
capture Csa = (Asas/Vsas) *Fucsf  ; // SAS in ng/mL 
...
[ CAPTURE ] Cvenous Cp Cpm Mball   Csa Csam Ceca Cec1 Cec2 Cec3 Cec4 Icf1 Icf2 Icf2 
