...
mdem <- read.csv("/Volumes/GoogleDrive/My Drive/PBPK/MRG solve PBPK/Data Extractions/Morphine/meineke_dem_dose.csv") #IDs, IV rates, Dose, BW and groupings

mBWm<- mean(mdem$BW)
mBWsd<- sqrt(var(mdem$BW))

set.seed(mBWm)
mBWd <- rnorm(10000,mBWm,22000)
mBWd <-mBWd[mBWd>(min(mdem$BW)) & mBWd<(max(mdem$BW))]


dosing<- mdem[-c(6,8)]
dem<-as.data.frame(mdem)


CLr <- rnorm(10000,30,20)
CLr<- CLr[CLr>1 ]
Kpf<- rnorm(10000,3,1)
Kpf<-Kpf[Kpf>1]
t_mein<- c(0.10,0.083333333,0.25,0.5,1,2,4,6,8,12,16,20,24)
...
repeat{
  for(i in 0:1000){ 
    ...
    idataCL<- expand.idata(CLrange=(sample(CLr,10,replace=FALSE)))
    idataBW<- expand.idata(BW=(sample(mBWd,10,replace=FALSE)))
    idataKpf<- expand.idata(Kpf=(sample(Kpf,10,replace=FALSE)))
    idata1<- merge(it,idataCL)
    idata2<- merge(idata1,idataBW)
    idata3<- merge(idata2,idataKpf)
    
    data<- merge(dosing,idata3)
    ...
    sp <- split(data2,data2$ID)#### population data set split into individual IDs to allow for parallel processing
    
    out<- future_lapply(sp, function(chunk) {  
      mod_mo<-loadso(mread("Morphine_091121b"))
      as.data.frame(
        mod_mo %>%
          data_set(chunk) %>%
          Req(Cp, Mball, Csa, Ceca,Cec1, Cec2, Cec3, Cec4, Icf1, Icf2, Icf3, Icf4, Bra, Cmva) %>%
          carry_out(x)%>%
          mrgsim(maxsteps=100000, tgrid=t_mein))})%>%
      bind_rows()
...
write.csv(NCA_tables,"/Volumes/GoogleDrive/My Drive/PBPK/MRG solve PBPK/outputs/Tables for Pub/morphine_ratios.csv",
          row.names=FALSE)
